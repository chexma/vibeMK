...
class ServiceHandler(BaseHandler):
    ...
    async def _get_services(self, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
                        ...
                        if isinstance(services, list):
                            ...
                            for service in services[:50]:  # Limit display
                                if isinstance(service, dict):
                                    ...
                                    plugin_output = extensions.get("plugin_output", "")[:50] + "..." if len(extensions.get("plugin_output", "")) > 50 else extensions.get("plugin_output", "No output")
        ...
        try:
            ...
            return self.error_response("Service retrieval failed", "Could not retrieve services using any available method")
    ...
    async def _get_service_status(self, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if result.get("success"):
                ...
                if isinstance(data, dict) and "extensions" in data:
                    ...
                    return self.error_response("Unexpected response format", f"Expected extensions in response, got: {list(data.keys()) if isinstance(data, dict) else type(data)}")
            else:
        ...
        try:
            ...
            livestatus_result = self.client.post("domain-types/bi_rule/actions/livestatus_query/invoke", 
                                               data={"query": livestatus_query})
    ...
    async def _discover_services(self, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if result.get("success"):
            ...
            return self.error_response("Discovery failed", f"Could not start service discovery for hosts '{hosts_text}'")