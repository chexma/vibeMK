...
class DebugHandler(BaseHandler):
    ...
    async def _test_all_host_endpoints(self, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
                    ...
                    if success:
                        ...
                        if isinstance(data, dict):
                            ...
                            if "extensions" in data:
                                ...
                                monitoring_indicators.extend([k for k in ext_keys if any(word in k.lower() for word in ["state", "status", "check", "plugin", "last"])])
                            
                            if "value" in data and isinstance(data["value"], list):
                                value_count = len(data["value"])
                                if data["value"] and isinstance(data["value"][0], dict):
                                    sample_keys = list(data["value"][0].keys())
                                    monitoring_indicators.extend([k for k in sample_keys if any(word in k.lower() for word in ["state", "status", "check", "plugin", "last"])])
                                results.append(f"   ✅ {method} Query {i+1}: {value_count} items, Keys: {keys}, Sample: {sample_keys[:5]}")
                            else:
                            ...
                            if monitoring_indicators:
                                ...
                                if "extensions" in data and "state" in str(data["extensions"]):
                                    ...
                                    if "extensions" in sample_item:
                                        results.append(f"      📊 Sample extensions: {str(sample_item['extensions'])[:200]}...")
                        else:
        ...
        return [{
            "type": "text",
            "text": (
                f"🔍 **Host Endpoints Test for: {host_name}**\\n" +
                "\\n".join(results)
            )
        }]
    ...
    async def _debug_host_data_structure(self, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
                        ...
                        if "value" in response_data and isinstance(response_data["value"], list):
                            ...
                            if target_host:
                                ...
                                if "extensions" in target_host:
                                    ...
                                    results.append(f"   🔧 Extensions keys: {list(ext.keys()) if isinstance(ext, dict) else type(ext)}")
                                    
                                    # Look for state/status information
                                    state_fields = [k for k in ext.keys() if any(word in k.lower() for word in ["state", "status", "check", "plugin", "last", "output"]) ] if isinstance(ext, dict) else []
                                    if state_fields:
        ...
        return [{
            "type": "text",
            "text": (
                f"🔬 **Host Data Structure Analysis for: {host_name}**\\n" +
                "\\n".join(results)
            )
        }]
    ...
    async def _debug_service_data_structure(self, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        return [{
            "type": "text",
            "text": (
                f"🔬 **Service Data Structure Analysis**\\n" +
                "\\n".join(results)
            )
        }]
            ...
            try:
                ...
                if success:
                    ...
                    if "value" in data:
                        ...
                        results.append(f"   ✅ Access granted - structure: {list(data.keys()) if isinstance(data, dict) else type(data)}")
                else:
                    error_data = result.get("data", {})
                    error_title = error_data.get("title", "Unknown error") if isinstance(error_data, dict) else str(error_data)
                    results.append(f"   ❌ Access issue: {error_title}")
                    
                    # Check for permission-related errors
                    if any(word in str(error_data).lower() for word in ["permission", "forbidden", "unauthorized", "access"]):
